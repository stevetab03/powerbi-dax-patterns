...
def fetch_spot(start: str, end: str) -> pd.DataFrame:
    ...
    logger.info("Spot: %d rows (%s to %s)",
...
def fetch_futures(start: str, end: str) -> pd.DataFrame:
    ...
    raw = yf.download(FUTURES_TICKER, start=start, end=end,
    ...
    logger.info("Futures: %d rows (%s to %s)",
                len(df), df["date"].min().date(), df["date"].max().date())
    return df
...
def build_expiry_calendar(start_year: int, end_year: int) -> pd.DataFrame:
    ...
    for year in range(start_year, end_year + 1):
        for month in range(1, 13):
            ...
            rows.append({"contract_month": f"{year}-{month:02d}",
                         "expiry_date": expiry})
...
def build_basis_panel(spot: pd.DataFrame, futures: pd.DataFrame,
    ...
    panel["tau_bin"]       = pd.cut(panel["tau_days"].astype(float),
                                    bins=TAU_BINS, labels=TAU_LABELS,
                                    right=True).astype(str)
    panel["basis_var_21d"] = panel["basis"].rolling(ROLLING_WINDOW, min_periods=10).var()
...
def build_term_structure(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    spot = panel[["date", "spot_price", "year",
                  "month_number", "month_short", "period_label"]].copy()
    spot = spot.rename(columns={"spot_price": "price"})
    spot["series"] = "Spot"
    spot["sort_order"] = 1

    fwd = panel[["date", "front_price", "year",
                 "month_number", "month_short", "period_label"]].copy()
...
def export(panel, var_tau, term_struct, events,
